...
function getLongDateTime($unix, $unix_end, $isAllDay, $date_type, $lan, $lan_arr_, $lan_arr_eosa_) {
    ...
    // 1 - Da lunedi 15 dicemre 2014 ore 15:30 a martedì 28 dicembre 2014 ore 24:00 (lunedi 15 dicembre 2014 dalle 15:30 fino alle 01:30) Sunday 15 june 2002 time 15:24
    ...
    if($date_type=='1' || $date_type=='2' || $date_type=='3'){ 
        ...
        if ($isOneDay==true){
            ...
            if(($date_type=='1') || ($date_type=='2')){   
                ...
                if ($isAllDay=='no'){
                    //if with time
                    $final_date = $lan_arr_eosa['from'] . ' ' . $final_date . ' ' . $lan_arr_eosa['time'] . ' ' . $hours . ':'. $min . ' ' . $lan_arr_eosa['to'] . ' ' . $fullDay_e . ' ' . $DayOfmonth_e . ' ' .$fullmonth_e.' ' . $year_e . ' ' . $lan_arr_eosa['time'] . ' ' . $hours_e . ':'. $min_e; //Da lunedi 15 dicemre 2014 ore 15:30 a martedì 28 dicembre 2014 ore 24:00 || Da 15/12/14 ore 15:30 a 21/12/14 ore 24:00  
                } else $final_date = $lan_arr_eosa['from'] . ' ' . $final_date . ' ' . $lan_arr_eosa['to'] . ' ' . $fullDay_e . ' ' . $DayOfmonth_e . ' ' .$fullmonth_e.' ' . $year_e; //Da lunedi 15 dicembre 2014 a martedì 28 dicembre 2014
            }
            if(($date_type=='3') || ($date_type=='3')){ 
                if ($isAllDay=='no'){
                    //if with time
                    $final_date = $lan_arr_eosa['from'] . ' ' . $final_date . ' ' . $lan_arr_eosa['time'] . ' ' . $hours . ':'. $min . ' ' . $lan_arr_eosa['to_2'] . ' ' . $DayOfmonth_e . '/' . $month_e . '/' .  date('y',$unix_end) . ' ' . $lan_arr_eosa['time'] . ' ' . $hours_e . ':'. $min_e; //Da lunedi 15 dicemre 2014 ore 15:30 a martedì 28 dicembre 2014 ore 24:00 || Da 15/12/14 ore 15:30 a 21/12/14 ore 24:00  
                } else $final_date = $lan_arr_eosa['from'] . ' ' . $final_date . ' ' . $lan_arr_eosa['to_2'] . ' ' . $DayOfmonth_e . '/' . $month_e . '/' .  date('y',$unix_end); //Da lunedi 15 dicembre 2014 a martedì 28 dicembre 2014 || Da 15/12/14 a 19/12/14
            }
        }
    }
...
function readEventData($date_out, $date_in, $lan, $optionArr) {
    ...
    if (($cover == "")||($cover == "main")||($url == "")) { 
        //Default
        $urlArr = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), array( 1000,800 ),true, '' );
        $url = $urlArr[0];
    }
    ...
    //5 - Location
    $loca = get_post_meta($id,'evcal_location', true);
    if (strlen($loca) == 0) $loca = "--";
    array_push($item_array,$loca);
    
    //6 - Organizer
    array_push($item_array,get_post_meta($id,'evcal_organizer', true));
    
    //7 - Small Description
    array_push($item_array, HTMLtoText(get_the_content(),500));
    
    //8 - Long Date with Time
    $isAllDay = get_post_meta($id,'evcal_allday', true); 
    $unix = get_post_meta($id, 'evcal_srow', true);
...
function inDateRange($date_code, $featured, $metaquery)
{
    $all = true;
    $ret = false;
    $idx = 0;
    ...
    if (isset($metaquery[0]["key"])) 
    {
        if ($metaquery[0]["key"] == "_featured")
        {
            $all = $metaquery[$idx]["value"] == $featured;
            if (! $all) return false;
            $idx++;
        }
    }
    ...
    //{
    if (is_null($metaquery[$idx]["relation"]))
    {
...
function createEventRepeats($slidertype,$date_out,$date_in,$lan,$metaquery,& $event,$optionArr) {
    global $lan_arr;
    global $lan_arr_eosa;

    $arrRepeats = array();
    $ev_vals = get_post_custom(get_the_Id());
        ...
        if(!empty($repeat_intervals) && is_array($repeat_intervals)) { 
            ...
            foreach($repeat_intervals as $interval) {
                if ($count>0) {
                    ...
                    $isAllDay = get_post_meta(get_the_Id(),'evcal_allday', true); //yes/n
                    $unix = $E_start_unix;
                    $unix_end = $E_end_unix;
                    ...
                    //9 - Long Date with Time 2
                    $event[9] = getLongDateTime($unix,$unix_end,$isAllDay,$date_in,$lan,$lan_arr, $lan_arr_eosa);
                    $event[11] = get_permalink(get_the_Id()) . "?l=" . $lan . "&ri=" . $count;
                    $event[15] = $E_start_unix;
                    $featured = get_post_meta(get_the_Id(), '_featured', true);
                    $in = inDateRange($event[15], $featured, $metaquery);
