        ...
        <div class="eventon_sorting_section">
            <?php if (($filters == "all")||($filters == "order")) {  ?>
                ...
                <div class="evo_sortby">
                    <p><?php echo $lan_arr['evcal_lang_sort'] ?></p>
                </div>
                <div class="evo_srt_sel" eid="<?php echo $id; ?>" sort-value="number">
                    <p class="eo_label"><?php echo $lan_arr['evcal_lang_sdate'] ?><i class="fa fa-caret-down"></i></p>
                    <div class="evo_srt_options">
                        <p sort-value="number" eid="<?php echo $id; ?>" class="evs_btn evs_hide"><?php echo $lan_arr['evcal_lang_sdate'] ?></p>
                        <p sort-value="title" eid="<?php echo $id; ?>" class="evs_btn "><?php echo $lan_arr['evcal_lang_stitle'] ?></p>
                    </div>
                </div>
                <div class="clear"></div>
            </div>
           ...
           ?>
            <div class="eventon_filter_line">
                <div class="eventon_filter_selection">
                    <p class="eo_label cat_all_button" eid="<?php echo $id; ?>"><?php echo $lan_arr['evcal_lang_all'] ?></p>
                </div>
                <?php 
                   ...
                   ?>
                        <div class="eventon_filter_selection" id="box_event_type<?php echo $et_; ?>">
                            <p class="eo_label" la="<?php echo $et_; ?>" eid="<?php echo $id; ?>" id="event_type<?php echo $et_; ?>"><?php echo $eo_cat[$i][0][0]; ?></p>
                            <div class="eventon_filter_dropdown" id="eo_dropdown<?php echo $et_; ?>" style="display: none">
                                <div class="catn"><?php echo $eo_cat[$i][0][0]; ?></div>
                                <?php  for ($j=0; $j < count($eo_cat[$i][1]); $j++) { 
                                           $tmp = $eo_cat[$i][1][$j];
                                 ?>
                                <p cat-slug="<?php echo $tmp["slug"]; ?>" la="<?php echo $et_; ?>" eid="<?php echo $id; ?>"><?php echo $tmp["name"]; ?></p>
                                <?php  }  ?>
                            </div>
                        </div>
              <?php  }  ?>
