...
function ajax_getEvents(type, idslider, lan_arr, lan_arr_eosa, paged, eid) {
    var arr_shortcode = window[idslider + '_eo_js_array_sc'];
    jQuery.post(ajaxURL, { 'action': 'ajax_getEvents', 'arrSc': arr_shortcode, 'lan_arr': lan_arr, 'lan_arr_eosa': lan_arr_eosa, 'paged': paged }, function (response) {
        if (type == "masonry") {
...
function fillEventEOSA(index, id, main_array) {
    ...
    jQuery('#' + id + '_eo_location').html(main_array[index][5]);
    ...
    else jQuery('#' + id + '_eo_organizer_box').css('display', 'none');
...
function eosa_original_event_init(plugin_url) { 
    if (getURLParameter("eosa_original_page") == "yes") {
        var cssLink = jQuery("<link rel='stylesheet' type='text/css' href='" + plugin_url + "/eventon-slider-addon/assets/css/style.css'>");
...
function isTextOverflow(id) {
    //devo chiamarlo dopo che l'elemento è stato reso visibile
