...
function ajax_getEvents()  {
    ...
    $args = array( 
        ...
        'posts_per_page' => intval($arrSc[12]),
        'paged'=> $paged,
        'order' => $arrSc[11], 
        'meta_key' => 'evcal_srow', 
        'orderby' => 'meta_value', 
    ...
    while(have_posts()){
        ...
        $in = inDateRange($event[15],$featured,$arrSc[18]);
        $metaquery = $arrSc[18];
        
        if ($in) {
          if ($arrSc[0] == "masonry") array_push($arrEvents, $event);
        }

        /* MIKE */        
        $arrRepeats = createEventRepeats($arrSc[0],$arrSc[14], $arrSc[15],$arrSc[16],$arrSc[18],$event,$optionArr);
        $arrEvents = array_merge($arrEvents,$arrRepeats);
        /* ---- */ 
    }
    
    die(json_encode_arr($arrEvents));
}
...
function register_my_script() {
    global $plugin_url;
	wp_register_style('eosa-main-style', $plugin_url.'/eventon-slider-addon/assets/css/style.css', null, true );
    ...
    wp_register_script('evcal_gmaps', 'https://maps.googleapis.com/maps/api/js?sensor=false', array(), null, true);
...
function eosa_original_event() {   
    global $plugin_url; 
     echo "<script>
             jQuery('link[href=\'link/to/css\']').remove(); jQuery(document).ready(function() { eosa_original_event_init('" .$plugin_url."'); });
         </script>";
...
<?php if ($eo_index == 0) { ?> 
<style type="text/css">
    #<?php echo $id; ?>, #eosa_loader_<?php echo $id; ?> {
        display: none;
    }
</style>
